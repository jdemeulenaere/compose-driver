...
echo "Starting Android Application via Compose Driver..."
echo "Logs will be written to $LOG_FILE"

# Start the gradle task in the background using the plugin-generated project.
# We pass the composable to run.
./gradlew :compose-driver-android:run -Dcompose.driver.composable=io.github.jdemeulenaere.compose.driver.sample.android.AndroidApplicationKt.AndroidApplication > "$LOG_FILE" 2>&1 &
PID=$!
